...
| `/home/<user>/.openclaw/` | OpenClaw user directory (detection trigger: `openclaw.mjs`) |
...
`install.sh` / `upgrade.sh` automatically installs and enables `openclaw-gateway.service` when it detects `~/.openclaw/openclaw.mjs`. No manual copy is needed.
...
OPENCLAW_MJS=/path/to/your/openclaw/dist/index.js
```
...
```bash
# Replace:  exec node "$OPENCLAW_MJS" gateway
...
- **OpenClaw binary path fix** — `install.sh` and `upgrade.sh` now detect OpenClaw via `~/.openclaw/openclaw.mjs` instead of the incorrect `~/.npm-global/bin/openclaw`
