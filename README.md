...
`install.sh` / `upgrade.sh` automatically installs and enables `openclaw-gateway.service` when it detects `~/.openclaw/dist/index.js` (or legacy `~/.openclaw/openclaw.mjs`). No manual copy is needed.
...
The default configuration assumes a Node-based OpenClaw install in `~/.openclaw`, with the preferred entry point at `~/.openclaw/dist/index.js`. For unusual layouts, override it with `OPENCLAW_ENTRY` (or legacy `OPENCLAW_MJS`) in the service environment:
