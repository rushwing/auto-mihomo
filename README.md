...
`install.sh` / `upgrade.sh` automatically installs and enables `openclaw-gateway.service` when it detects `~/.openclaw/openclaw.njs`. No manual copy is needed.
...
- **OpenClaw binary path fix** — `install.sh` and `upgrade.sh` now detect OpenClaw via `~/.openclaw/openclaw.njs` instead of the incorrect `~/.npm-global/bin/openclaw`
