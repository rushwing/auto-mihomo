...
│   └── openclaw-gateway.service    # OpenClaw gateway reference unit
├── skill/
...
A systemd unit for OpenClaw gateway is included at `systemd/openclaw-gateway.service`. It uses `Wants=mihomo.service` (not `Requires=`) so that restarting Mihomo during a config reload does not cascade-stop OpenClaw and trigger a restart loop. It reads proxy settings from `/etc/auto-mihomo/proxy.env` (auto-generated by `update_sub.sh`), so port changes propagate automatically.

`install.sh` / `upgrade.sh` automatically installs and enables `openclaw-gateway.service` when it detects `$USER_HOME/.openclaw/dist/index.js` (or legacy `$USER_HOME/.openclaw/openclaw.mjs`). No manual copy is needed.

When the system service is installed, the scripts also mask `$USER_HOME/.config/systemd/user/openclaw-gateway.service` by default to prevent the upstream OpenClaw CLI from re-enabling a conflicting `systemctl --user` gateway on port `18789`. Set `AUTO_MIHOMO_MASK_OPENCLAW_USER_UNIT=0` if you explicitly want to keep the user unit path available.

The service uses a wrapper (`scripts/start_openclaw_with_proxy.sh`) that runs `scripts/update_sub.sh` first, refreshes the generated `/etc/auto-mihomo/proxy.env`, then starts the OpenClaw gateway with `dist/index.js gateway --port ...` (falling back to `openclaw.mjs` only for older installs). The systemd unit also injects the standard OpenClaw service metadata (`OPENCLAW_GATEWAY_PORT`, `OPENCLAW_SYSTEMD_UNIT`, `OPENCLAW_STATE_DIR`, `OPENCLAW_CONFIG_PATH`, etc.) so the newer gateway runtime keeps the expected state/config context.

`proxy-bootstrap.cjs` is kept only as an optional manual fallback. It now uses `EnvHttpProxyAgent` and does not override `globalThis.fetch`, but the default unit no longer preloads it because recent OpenClaw releases already honor the standard proxy environment variables:
...
The default configuration assumes a Node-based OpenClaw install in `~/.openclaw`, with the preferred entry point at `$USER_HOME/.openclaw/dist/index.js`. For unusual layouts, override it with `OPENCLAW_ENTRY` (or legacy `OPENCLAW_MJS`) in the service environment:

```bash
# for example in a systemd drop-in or temporary shell:
OPENCLAW_ENTRY=/path/to/your/openclaw/dist/index.js
```
...
```bash
# Replace:  exec "$NODE_BIN" "$OPENCLAW_ENTRY" gateway --port "$OPENCLAW_PORT"
exec /usr/local/bin/openclaw gateway --port "$OPENCLAW_PORT"
```
...
- **OpenClaw system/user 冲突防护** — `install.sh` / `upgrade.sh` 在安装 system 级 `openclaw-gateway.service` 后，会默认 mask `$USER_HOME/.config/systemd/user/openclaw-gateway.service`，避免 OpenClaw 官方 CLI fallback 到 `systemctl --user` 时再次拉起冲突实例并抢占 18789 端口
