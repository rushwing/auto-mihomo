...
cd auto-mihomo
sudo bash install.sh --user openclaw    # specify service user
```
...
A reference systemd unit for OpenClaw gateway is included at `systemd/openclaw-gateway.service`. It depends on `mihomo.service` and reads proxy settings from `/etc/auto-mihomo/proxy.env` (auto-generated by `update_sub.sh`), so port changes propagate automatically.
